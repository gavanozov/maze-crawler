...
class Leaderboard:
    ...
    def add_new_score(self, difficulty, score):
        ...
        if len(scores) < 10:
            name = input("You have reached TOP 10 in the leaderboard! Enter your name: ")
    ...
    def print_leaderboard(self, difficulty):
        if difficulty not in self.leaderboard:
            raise ValueError(f"Invalid difficulty: {difficulty}")
        print(f"\n{difficulty.capitalize()} Leaderboard:")
        for rank, entry in enumerate(self.leaderboard[difficulty], start=1):
